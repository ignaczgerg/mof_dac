...
def _gather_metadata_and_position_stats(dataset: Dataset[BaseData], num_workers: int, batch_size: int):
    loader = DataLoader(
        ...
        batch_size=batch_size,
        num_workers=num_workers,
        collate_fn=lambda data_list: [(data.pos_relaxed.shape[0], data.pos_relaxed.numpy()) for data in data_list],
        shuffle=False,
    )
    ...
    for batch in tqdm(loader, total=len(loader)):        
        for natoms, pos_relaxed in batch:
            natoms_list.append(natoms)
            total_sum += pos_relaxed.sum()
            total_sq_sum += (pos_relaxed ** 2).sum()
            count += pos_relaxed.size  # total elements = num_atoms * 3
...
def main():
    ...
    print("Relaxed Position mean:", pos_mean)
    print("Relaxed Position std:", pos_std)

    np.savez(dest, natoms=natoms, relaxed_pos_mean=pos_mean, relaxed_pos_std=pos_std)
