...
class AtomBucketBatchSampler(BatchSampler):
    ...
    def __iter__(self):
        # NOTE: Do NOT auto-increment _epoch here. Lightning's _DataFetcher calls
        # len(sampler) BEFORE iter(sampler), so auto-incrementing causes __len__
        # and __iter__ to return different batch counts. When actual < expected,
        # data_fetcher.done is never True → is_last_batch is False → validation
        # is silently skipped. Epoch management must be external via set_epoch().
