...
class PositionNormalizationConfig(TypedConfig):
    """
    Handles standard, log, and log1p normalization.

    Attributes:
        mean (float): Mean used for standardization.
        std (float): Standard deviation used for standardization.
        normalization_type (str): Either 'standard', 'log', or 'log1p'.
    """

    type: Literal["pos"] = Field(default="pos")
    mean: float = 1.0
    std: float = 1.0
    normalization_type: Literal["standard", "log", "log1p"] = "standard"

    def normalize(self, value: torch.Tensor) -> torch.Tensor:
        if self.normalization_type == "log":
            value = torch.log1p(value)  # Apply log(1+x) transformation
    ...
    def denormalize(self, value: T, center: T = None) -> T:
        ...
        if self.normalization_type == "log" or self.normalization_type == "log1p":
            value = torch.expm1(value)  # Reverse log(1+x) transformation
...
class NormalizationConfig(TypedConfig):
    ...
    Attributes:
        ...
        normalization_type (str): Either 'standard', 'log', or 'log1p'.
          - 'standard': (x - mean) / std
          - 'log': log(|x|) standardization (for wide-range data, handles negative values)
          - 'log1p': log(1+|x|) standardization (more stable for small values)
    """
    ...
    def normalize(self, value: torch.Tensor) -> torch.Tensor:
        ...
        if self.normalization_type == "log":
            ...
            # log1p is stable for all values (positive, negative, and near-zero)
