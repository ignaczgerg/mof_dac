...
class DatasetSampleNConfig(TypedConfig):
    ...
    If int (or float > 1): treated as absolute sample count.
    If float in (0, 1]: treated as percentage of the dataset (e.g., 0.2 = 20%)."""

    seed: int
    """Seed for the random number generator used to sample the dataset"""

    max_natoms: int | None = None
    """Maximum number of atoms allowed in a sample (used for filtering before sampling)"""
...
def wrap_common_dataset(dataset: TDataset, config: CommonDatasetConfig) -> TDataset:
    if (sample_n := config.sample_n) is not None:
        n_value = sample_n.sample_n
        
        # Convert percentage to absolute count if needed
        if isinstance(n_value, float) and 0 < n_value <= 1:
            # It's a percentage - calculate absolute count from dataset size
            dataset_size = len(dataset)
            n_value = int(n_value * dataset_size)
        else:
            # Absolute count (convert to int if it's a float > 1)
            n_value = int(n_value)
        ...
        dataset = DT.sample_n_transform(
            ...
            n=n_value,
            seed=sample_n.seed,
            max_natoms=sample_n.max_natoms
        )
