...
#TODO: nn.RMSNorm is not available in torch 2.2.2
class RMSNorm(nn.Module):
    def __init__(self, normalized_shape, eps: float = 1e-8, elementwise_affine: bool = True):
        super().__init__()
        self.eps = eps
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Compute the RMS value over the last dimension
...
class RMSNormGraph(nn.Module):
    def __init__(self, hidden_size: int, **kwargs):
        super().__init__()
        self.node_norm = RMSNorm(hidden_size, **kwargs)
        self.edge_norm = RMSNorm(hidden_size, **kwargs)
...
def get_normalization_layer(normalization_type: NormalizationType, is_graph=True):
    return {
        ...
        NormalizationType.RMSNorm: RMSNormGraph if is_graph else RMSNorm,
    }[normalization_type]
