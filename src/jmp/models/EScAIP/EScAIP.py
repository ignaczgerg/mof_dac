...
from ..base import GraphModelMixin, HeadInterface

from .config import (
    EScAIPBackboneConfig, 
    GlobalConfigs,
...
class EScAIPBackbone(nn.Module, GraphModelMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.global_cfg: GlobalConfigs = config.global_cfg
        self.molecular_graph_cfg: MolecularGraphConfigs = config.molecular_graph_cfg
        self.gnn_cfg: GraphNeuralNetworksConfigs = config.gnn_cfg
        self.reg_cfg: RegularizationConfigs = config.reg_cfg

        # for trainer
        self.regress_forces = self.global_cfg.regress_forces
        self.use_pbc = self.molecular_graph_cfg.use_pbc
