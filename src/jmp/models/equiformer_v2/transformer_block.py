...
class SO2EquivariantGraphAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if causal_mask != None:
            alpha = alpha.masked_fill(causal_mask.unsqueeze(-1), float('-inf'))
            
        alpha = torch.nan_to_num(torch_geometric.utils.softmax(alpha, edge_index[1]), 0.0)
