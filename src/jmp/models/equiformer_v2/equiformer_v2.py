...
class MultiScalarRBFEncoder(nn.Module):
    """
    Per-feature RBF expansion for multiple scalar descriptors.

    Each feature gets its own learnable centers and gamma, so features
    with different scales (e.g. PLD in Å vs density in g/cm³) are
    handled independently.

    Args:
        num_features: Number of input scalar features (F).
        num_rbf_per_feature: RBF centers per feature (R).

    Input:  [B, F]
    Output: [B, F * R]
    """
    def __init__(self, num_features: int, num_rbf_per_feature: int = 16):
        super().__init__()
        self.num_features = num_features
        self.num_rbf = num_rbf_per_feature

        # Per-feature centers: [F, R]
        centers = torch.linspace(-1.0, 1.0, num_rbf_per_feature)
        centers = centers.unsqueeze(0).expand(num_features, -1).clone()
        self.centers = nn.Parameter(centers)

        # Per-feature log-gamma: [F]
        self.log_gamma = nn.Parameter(torch.zeros(num_features))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: [B, F] multi-feature descriptors
        returns: [B, F * R]
        """
        # Per-feature normalization (each feature to ~unit scale)
        scale = x.abs().mean(dim=0).clamp_min(1e-6)  # [F]
        x_norm = x / scale  # [B, F]

        # RBF expansion: [B, F, 1] - [1, F, R] -> [B, F, R]
        diff = x_norm.unsqueeze(-1) - self.centers.unsqueeze(0)
        gamma = torch.exp(self.log_gamma).view(1, -1, 1)  # [1, F, 1]
        rbf = torch.exp(-gamma * diff.pow(2))  # [B, F, R]

        return rbf.reshape(x.size(0), -1)  # [B, F*R]
...
class EquiformerV2Backbone(nn.Module, GraphModelMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if fusion_cfg and fusion_cfg.get("enabled", False) and fusion_cfg.get("fusion_type") == "early":
            feature_names = fusion_cfg.get("feature_names", [])
            num_features = len(feature_names)
            hidden_dim = int(fusion_cfg.get("hidden_dim"))
            num_rbf_per_feat = max(1, hidden_dim // num_features)
            total_rbf_dim = num_features * num_rbf_per_feat

            # Multi-scalar → per-feature RBF
            self.mof_rbf_encoder = MultiScalarRBFEncoder(num_features, num_rbf_per_feat)

            # RBF → sphere_channels (same output dim as L=0 embedding)
            self.mof_embedding_proj = nn.Sequential(
                nn.Linear(total_rbf_dim, self.sphere_channels),
                nn.SiLU(),
        ...
        # CHARGE EARLY FUSION
        # ----------------------------------
        self.charge_rbf_encoder = None
        self.charge_embedding_proj = None
        if getattr(config, "use_charge_embedding", False):
            self.charge_rbf_encoder = ScalarRBFEncoder(num_rbf=32)
            self.charge_embedding_proj = nn.Sequential(
                nn.Linear(32, self.sphere_channels),
                nn.SiLU(),
                nn.Linear(self.sphere_channels, self.sphere_channels),
            )
        # ------------------------
    ...
    def forward(self, data) -> dict[str, torch.Tensor]:
        ...
        if (
            ...
            # Get features: [Batch_Size, num_features]
            global_features = data.mof_descriptor

            # Multi-scalar → per-feature RBF → fusion embedding [B, sphere_channels]
            rbf = self.mof_rbf_encoder(global_features)            # [B, F*R]
        ...
        # ----------------------------------
        if (
            self.charge_rbf_encoder is not None
            and self.charge_embedding_proj is not None
            and hasattr(data, "partial_charges")
        ):
            charges = data.partial_charges  # [N_atoms]
            charge_rbf = self.charge_rbf_encoder(charges)        # [N_atoms, 32]
            charge_emb = self.charge_embedding_proj(charge_rbf)  # [N_atoms, sphere_channels]
            x.embedding[:, 0, :] = x.embedding[:, 0, :] + charge_emb
        # --- END CHARGE EARLY FUSION
