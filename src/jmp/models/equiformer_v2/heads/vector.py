...
if TYPE_CHECKING:
    ...
    from ...base import BackboneInterface


# @registry.register_model("equiformerV2_vector_head")
class EqV2VectorHead(nn.Module, HeadInterface):
    def __init__(self, backbone, hidden_channels_override: int = None, output_name: str = "forces"):
        super().__init__()
    ...
    def forward(self, data: Batch, emb: dict[str, torch.Tensor]):
        if self.activation_checkpoint:
            ...
            output = self.force_block(
                ...
                causal_mask=getattr(emb, "causal_mask", None),
            )
