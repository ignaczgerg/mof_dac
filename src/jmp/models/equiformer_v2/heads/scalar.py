    ...
    def forward(self, data: Batch, emb: dict[str, torch.Tensor | GraphData]):
        ...
        if self.output_channels > 1:
            ...
            for c in range(self.output_channels):
                output[:, c].index_add_(0, batch_idx, node_output[:, c])
        else:
            ...
            output.index_add_(0, batch_idx, node_output)

        if self.reduce == "sum":
            return {self.output_name: output / self.avg_num_nodes}
        elif self.reduce == "mean":
            if self.output_channels > 1:
                return {self.output_name: output / natoms.unsqueeze(-1)}
            return {self.output_name: output / natoms}
        else:
            raise ValueError(
...
class EqV2ClassificationHead(nn.Module, HeadInterface):
    ...
    def forward(self, data: Batch, emb: dict[str, torch.Tensor | GraphData]):
        ...
        graph_logits = scatter(
            node_logits,             # [N_real_nodes, num_classes]
            batch_idx,               # [N_real_nodes] graph‐idx for each node
            dim=0,
