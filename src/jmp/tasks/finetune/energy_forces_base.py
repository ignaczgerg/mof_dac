...
class EnergyForcesModelBase(
    FinetuneModelBase[TConfig, TModel], nn.Module, ABC, Generic[TConfig, TModel]
):
    @override
    ...
    def construct_energy_head(self):
        ...
        match self.config.model_cls:
            ...
            case models.EquiformerV2ModelWrapper:
                self.out_energy = EquiformerV2EnergyHead(
                    self.backbone, hidden_channels_override=256
                )
    ...
    def construct_output_heads(self):
        ...
        if (
            ...
            match self.config.model_cls:
                ...
                case models.EquiformerV2ModelWrapper:
                    self.out_forces = EquiformerV2ForceHead(
                        self.backbone, hidden_channels_override=128
                    )
    ...
    def forward(self, data: BaseData):
        ...
        with ExitStack() as stack:
            ...
            # TODO: support forward override for model wrappers
            if self.config.args.model_name == "gemnet":
                h = self.embedding(atomic_numbers)
                out: GOCBackboneOutput = self.backbone(data, h=h)
            elif self.config.args.model_name == "equiformer_v2":
