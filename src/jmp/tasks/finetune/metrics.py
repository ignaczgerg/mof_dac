...
class MetricsConfig(TypedConfig):
    ...
    report_r2: bool = False
    """Whether to report R² (coefficient of determination) in addition to MAE"""
...
class FinetuneMetrics(nn.Module):
    ...
    def __init__(
        ...
        if self.config.report_normal_metrics:
            ...
            if self.config.report_r2:
                self.r2s = TypedModuleDict(
                    {
                        target: torchmetrics.R2Score()
                        for target in self.regression_targets
                    },
                    key_prefix="r2_",
                )
    ...
    def forward(self, batch: BaseData, preds: dict[str, torch.Tensor]):
        ...
        if self.config.report_normal_metrics:
            ...
            if self.config.report_r2:
                for key, r2 in self.r2s.items():
                    if (mp := self.provider(key, batch, preds)) is None:
                        continue

                    r2.update(mp.predicted, mp.ground_truth)
        ...
        if self.config.report_log_metrics:
            for key in self.regression_targets:
                ...
                if self.config.report_r2:
                    self.log_r2s[key].update(log_pred, log_target)
                    metrics[f"{key}_log_r2"] = self.log_r2s[key]
        ...
        for t_idx, task in enumerate(tasks):
            ...
            for target in self.regression_targets(task):
                ...
                if config.report_normal_metrics:
                    ...
                    if config.report_r2:
                        self._metrics[prefix + target + "_r2"] = torchmetrics.R2Score()
                ...
                if config.report_log_metrics:
                    ...
                    if config.report_r2:
                        self._metrics[prefix + target + "_log_r2"] = torchmetrics.R2Score()
        ...
        for t_idx, task in enumerate(self.tasks):
            ...
            if self.config.report_normal_metrics:
                for key in self.regression_targets(task):
                    ...
                    if mp is None:
                        continue
                    for suf in ["_mae", "_rmse", "_mape", "_smape", "_roi_smape", "_mse", "_r2"]:
                        mkey = prefix + key + suf
                        if mkey in self._metrics.keys():
                            if suf == "_roi_smape":
                                ...
                                # R2Score requires ≥2 samples to compute; use update()
                                # to accumulate state without triggering per-batch compute()
                                self._metrics[mkey].update(mp.predicted, mp.ground_truth)
                            else:
            ...
            if self.config.report_log_metrics:
                for key in self.regression_targets(task):
                    ...
                    log_pred   = symlog(mp.predicted)
                    log_target = symlog(mp.ground_truth)
                    for suf in ["_log_mae", "_log_rmse", "_log_mape", "_log_smape", "_log_mse", "_log_r2"]:
                        mkey = prefix + key + suf
                        if mkey in self._metrics:
                            if suf == "_log_r2":
                                self._metrics[mkey].update(log_pred, log_target)
                            else:
