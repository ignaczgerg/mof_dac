...
import jmp.tasks.finetune.hmof as hm
import sys, inspect
...
class HMofConfig(FinetuneConfigBase):
    """Finetune config for hMOF graph-level regression targets."""

    normalization: dict[str, NormalizationUnion] = Field(default_factory=dict, discriminator="type")

    graph_scalar_targets: list[HMofTarget] = []
    ...
    graph_scalar_reduction: dict[str, Literal["sum", "mean", "max"]] = {
        ...
        "qst_co2": "mean",
        "excess_co2": "mean",
        "uptake_binary_co2": "mean",
        "qst_binary_co2": "mean",
        "excess_binary_co2": "mean",
        "uptake_binary_n2": "mean",
        "qst_binary_n2": "mean",
        "excess_binary_n2": "mean",
...
@final
class HMofModel(FinetuneModelBase[HMofConfig, TModel]):
    """Model wrapper for hMOF finetuning."""
    ...
    @override
    def validate_config(self, config: HMofConfig):
        super().validate_config(config)
        for key in config.graph_scalar_targets:
            assert key in self.targets, f"{key} is not a valid hMOF target"
    ...
    def _make_loader(self, dataset, *, train: bool):
        ...
        # raise RuntimeError("DEBUG: entered HMofModel._make_loader")
        if world_size > 1:
