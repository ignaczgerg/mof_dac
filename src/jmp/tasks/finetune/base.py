...
from ...models.EScAIP.EScAIP import EScAIPBackbone
...
from jmp.tasks.finetune.model_wrapper_base import (
    ...
    GraphBinaryClassificationOutputHead,
    NodeVectorOutputHead,
    BackboneConfig, Backbone, BackboneOutput,
)
...
class FinetuneLmdbDatasetConfig(CommonDatasetConfig):
    ...
    def create_dataset(self, split):
        return LmdbDataset(src=self.src, metadata_path=self.metadata_path, args=self.args, split_name=split)
...
class FinetuneConfigBase(BaseConfig, Generic[TModel]):
    ...
    def regression_targets(self):
        ...
        return (
            self.node_vector_targets + self.graph_scalar_targets 
    ...
    normalization: dict[str, Union[NormalizationConfig, PositionNormalizationConfig]] = {}
...
class _FinetuneModelBase(LightningModuleBase[TConfig], Generic[TConfig, TModel]):
    @abstractmethod
    ...
    def __init__(self, hparams: TConfig):
        ...
        self.load_backbone_state_dict = self.model.load_backbone_state_dict
           
        
        self.train_metrics = FinetuneMetrics(
            self.config.metrics,
            self.metrics_provider,
            self.cls_metrics_provider,
            self.config.graph_scalar_targets,
            self.config.graph_classification_targets,
            self.config.graph_classification_adabin_targets,
        ...
        for cls_target in (self.config.graph_classification_targets + self.config.graph_classification_adabin_targets):
            match cls_target:
    ...
    def validate_config(self, config: TConfig):
        ...
        for a, b in itertools.combinations(
            [
                ...
                [target.name for target in config.graph_classification_adabin_targets],
    ...
    def metrics_provider(
        ...
        if (
            ...
            norm = self.config.normalization[prop]
        ...
        if (
            ...
            target = getattr(batch, f"{prop}_raw")
            # pred = self.bin_centers[prop][pred.argmax(dim=-1)]
            probs = F.softmax(pred, dim=-1)
            pred = (probs * self.bin_centers[prop].unsqueeze(0)).sum(dim=-1)

        return MetricPair(predicted=pred, ground_truth=target)
    
    def cls_metrics_provider(
        self,
        prop: str,
        batch: BaseData,
        preds: dict[str, torch.Tensor],
    ...
    def construct_graph_classification_output_head(
        ...
        match target:
            ...
            case MulticlassClassificationTargetConfig():
                return self.model.GraphMulticlassClassificationOutputHead(
                    self.config,
                    target,
                    self.backbone,
                    reduction=self.config.graph_classification_reduction.get(
    ...
    @abstractmethod
    def forward(self, data: BaseData) -> dict[str, torch.Tensor]:
        pass 
    ...
    def compute_losses(self, batch: BaseData, preds: dict[str, torch.Tensor]):
        ...
        match (loss_type := self.config.args.loss.lower()):
            case "l1":
                loss_fn = F.l1_loss
            case "huber":
                loss_fn = F.huber_loss
            case _:
                raise ValueError(f"Unsupported loss type: {loss_type}. Choose 'huber' or 'l1'.")

        for target in self.config.graph_scalar_targets:
            if loss_type == "huber":
                preds[target] = preds[target].to(torch.float32)
                batch[target] = batch[target].to(torch.float32)

            loss = loss_fn(preds[target], batch[target])
        ...
        for target in self.config.graph_classification_adabin_targets:
            raw_target = batch[f"{target.name}_raw"]
            probs = F.softmax(preds[target.name], dim=-1)
            pred = (probs * self.bin_centers[target.name].unsqueeze(0)).sum(dim=-1)
            # TODO: Experiment with MSE loss
            loss = F.l1_loss(pred, raw_target)
            self.log(f"{target.name}_reg_loss", loss)
            
            # TODO: Support scaling regularization loss
            coef = self.config.graph_scalar_loss_coefficients.get(
                target.name, self.config.graph_scalar_loss_coefficient_default
            )
            reg_coef = 1.0
            loss = reg_coef * coef * loss
            self.log(f"{target.name}_reg_loss_scaled", loss)
            losses.append(loss)
    ...
    def generate_graphs(
        ...
        if self.config.model_cls == models.GemNetModelWrapper:
            aint_graph = generate_graph(
                ...
                pbc=pbc, normalize_edge_vector=True
            )
    ...
    def _transform_cls_data(self, data: BaseData):
        ...
        """
        for target_config in (self.config.graph_classification_targets + self.config.graph_classification_adabin_targets):
            match target_config:
                case BinaryClassificationTargetConfig():
...
class FinetuneModelBase(_FinetuneModelBase[TConfig, TModel], Generic[TConfig, TModel]):
    ...
    def forward(self, data: BaseData):
        ...
        match self.config.model_cls:
            ...
            case models.SchNetModelWrapper:
                out = cast(SchNetBackboneOutput, self.backbone(data))

            case models.EScAIPModelWrapper:
                out = cast(EScAIPBackboneOutput, self.backbone(data))

            case models.EquiformerV2ModelWrapper | models.AutoregLlamaEquiformerV2ModelWrapper:
                out = cast(EquiformerV2BackboneOutput, self.backbone(data))
