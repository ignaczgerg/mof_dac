...
from ...models.equiformer_v2.equiformer_v2 import (
    ...
    EqV2ScalarHead, EqV2ClassificationHead, ScalarRBFEncoder, MultiScalarRBFEncoder
)
        ...
        def __init__(
            ...
            *,
            ...
            if fusion_cfg and fusion_cfg.get("enabled", False) and fusion_cfg.get("fusion_type") == "late":
                ...
                # Multi-scalar → per-feature RBF (self-normalizing)
                ...
                self.late_fusion_mlp = nn.Sequential(
                    nn.Linear(total_rbf_dim, hidden_dim),
                    nn.SiLU(),
            ...
            *,
            ...
            if self.heteroscedastic:
                ...
                if self.late_fusion_mlp is not None and self.mof_rbf_encoder is not None:
                    ...
                    global_feature = data.mof_descriptor.to(mean.device)  # [B, F]
                    rbf = self.mof_rbf_encoder(global_feature)  # [B, F*R]
            ...
            if self.late_fusion_mlp is not None and self.mof_rbf_encoder is not None:
                ...
                global_feature = data.mof_descriptor.to(energy.device)  # [B, F]
