...
from jmp.models.equiformer_v2.heads.node_sh import EqV2NodeSHHead
...
AdsorptionDbTarget: TypeAlias = Literal[
    ...
    'co2_uptake',
    'n2_uptake',
    # Geometric properties (used as labels when geometric_targets is set)
    'lcd', 'pld', 'gcd', 'unitcell_volume', 'density', 'asa', 'av', 'nav',
]
...
# ---------------------------------------------------------------------------
# Uptake unit conversion: mmol/g <-> alternative units
# ---------------------------------------------------------------------------
UPTAKE_TARGETS = {"co2_uptake", "n2_uptake"}

def _compute_cell_mol_weight(atomic_numbers: torch.Tensor) -> float:
    """Molecular weight of the unit cell (g/mol) from atomic numbers."""
    return float(atomic_masses[atomic_numbers.cpu().numpy()].sum())

def _convert_uptake_mmolg_to_per_cell(data: BaseData) -> BaseData:
    """Convert uptake from mmol/g to #molecules/cell (in-place).

    Formula: n_per_cell = uptake_mmolg * MW_cell / 1000
    Stores ``cell_mol_weight`` on *data* for the inverse transform.
    """
    mw = _compute_cell_mol_weight(data.atomic_numbers)
    data.cell_mol_weight = torch.tensor(mw, dtype=torch.float32)
    for target in UPTAKE_TARGETS:
        if hasattr(data, target):
            val = getattr(data, target)
            setattr(data, target, val * mw / 1000.0)
    return data

def _convert_uptake_mmolg_to_per_volume(data: BaseData) -> BaseData:
    """Convert uptake from mmol/g to #molecules/Ångström³ (in-place).

    Formula: n_per_volume = uptake_mmolg * MW_cell / (1000 * V_cell)
    Stores ``cell_mol_weight`` and ``cell_volume`` on *data* for the inverse transform.
    """
    mw = _compute_cell_mol_weight(data.atomic_numbers)
    data.cell_mol_weight = torch.tensor(mw, dtype=torch.float32)

    # Get unit cell volume (in Ångström³)
    if hasattr(data, "unitcell_volume"):
        vol = float(data.unitcell_volume)
    else:
        # Fallback: compute from cell matrix (Ångström³)
        cell = data.cell if data.cell.dim() == 2 else data.cell.squeeze(0)
        vol = float(torch.det(cell).abs().item())
    data.cell_volume = torch.tensor(vol, dtype=torch.float32)

    for target in UPTAKE_TARGETS:
        if hasattr(data, target):
            val = getattr(data, target)
            setattr(data, target, val * mw / (1000.0 * vol))
    return data

def _inverse_convert_uptake(
    pred: torch.Tensor,
    target: torch.Tensor,
    batch: BaseData,
    mask: torch.Tensor,
    ...
    """Convert uptake back to mmol/g for metric reporting.

    Args:
        pred: Predictions (in converted units if conversion_mode != "none")
        target: Ground truth (in converted units if conversion_mode != "none")
        batch: Data batch containing cell_mol_weight and optionally cell_volume
        mask: Boolean mask to select relevant samples
        conversion_mode: "none", "per_cell", or "per_volume"

    Returns:
        (pred_mmol_g, target_mmol_g): Values converted back to mmol/g
    """
    if conversion_mode == "per_cell":
        mw = batch.cell_mol_weight[mask]
        pred = pred * 1000.0 / mw
        target = target * 1000.0 / mw
    elif conversion_mode == "per_volume":
        mw = batch.cell_mol_weight[mask]
        vol = batch.cell_volume[mask]
        pred = pred * 1000.0 * vol / mw
        target = target * 1000.0 * vol / mw
    return pred, target
...
class AdsorptionFilter:
    ...
    def __call__(self, data: BaseData) -> bool:
        ok = (
            (data.co2_uptake > 0.0)
            and not math.isnan(data.qst_co2)
            # and not math.isnan(data.kh_co2)
            and (data.qst_co2 < 0.0)
            # and (data.kh_co2 > 0.0)
            and not math.isnan(getattr(data, 'unitcell_volume', float('nan')))
            and not math.isnan(getattr(data, 'density', float('nan')))
        )
...
class MOFFeatureConfig(TypedConfig):
    ...
    feature_names: list[str] = ["pld", "lcd", "gcd", "unitcell_volume", "density", "asa", "av", "nav"]
    fusion_type: str = "early"            # "early" or "late" fusion
    hidden_dim: int = 128                 # total hidden dimension after RBF encoding
...
class AdsorptionDbConfig(FinetuneConfigBase):
    ...
    'co2_uptake': 'mean',
    'n2_uptake': 'mean',
    # Geometric properties (when used as targets)
    'lcd': 'mean', 'pld': 'mean', 'gcd': 'mean',
    'unitcell_volume': 'mean', 'density': 'mean',
    'asa': 'mean', 'av': 'mean', 'nav': 'mean',
    }


    uptake_conversion_mode: Literal["none", "per_cell", "per_volume"] = "none"
    """Uptake unit conversion mode:
    - "none": No conversion, train on mmol/g (intensive property)
    - "per_cell": Convert to #molecules/cell (extensive, count per unit cell)
    - "per_volume": Convert to #molecules/Ångström³ (density per unit volume)
    All metrics and prediction logs are reported in mmol/g (inverse conversion applied)."""
    ...
    exclude_keys: list[str] = [
        ...
        "diff_qst_co2_h2o",
        "diff_qst_co2_n2",
        # void point preprocessing fields (consumed during ghost injection)
        "void_positions",
        "void_volumes",
        "num_void_points",
        "co2_sh_coeffs",
    ...
    node_sh_targets: list[str] = []
    node_sh_lmax: int = 4
    node_sh_loss_coefficient: float = 1.0
    node_sh_loss_type: Literal["mse", "l1"] = "mse"
    node_sh_l_weights: list[float] | None = None
...
class AdsorptionDbModel(FinetuneModelBase[AdsorptionDbConfig, TModel]):
    ...
    def __init__(self, hparams: AdsorptionDbConfig):
        ...
        # Geometric properties used as targets must NOT be excluded — they need to survive batching
        geo_as_targets = GEOMETRIC_PROPERTY_NAMES & set(self.config.graph_scalar_targets)
        
        if "adsorption_aramco" in task_names:
            self.config.exclude_keys.append("kh_co2")
        ...
        if not is_db1_solo:
            ...
            for k in db1_specific_keys:
                if k in geo_as_targets:
                    continue  # keep as label, don't exclude
                if k not in self.config.exclude_keys:
                    self.config.exclude_keys.append(k)
       
        if self.config.node_sh_targets:
            for k in ["co2_sh_coeffs", "has_co2_sh"]:
                if k in self.config.exclude_keys:
                    self.config.exclude_keys.remove(k)
    ...
    def _get_roi(self, key: str):
        ROI = {
            ...
            "pld": (float("-inf"), float("inf")),
            "lcd": (float("-inf"), float("inf")),
        }
    ...
    def _task_save_key(self, task: TaskConfig) -> str:
        ...
        if "adsorption_mof_db_anion_co2sh" in name: return "ANION_CO2SH"
        if "adsorption_mof_db_anion" in name: return "ANION"
    ...
    def step_prediction_logging(self, batch, preds, stage: str):
        ...
        for task_idx, task in enumerate(self.config.tasks):
            ...
            for target in self.config.graph_scalar_targets:
                ...
                pred_tensor = preds_cpu[target]
                pred_np = pred_tensor[..., task_idx].numpy().reshape(-1)
                ...
                # Convert uptake back to mmol/g for prediction logging
                if self.config.uptake_conversion_mode != "none" and target in UPTAKE_TARGETS:
                    mw_np = batch.cell_mol_weight[mask].detach().cpu().numpy()
                    if self.config.uptake_conversion_mode == "per_cell":
                        pred_np = pred_np * 1000.0 / mw_np
                        gt_np   = gt_np   * 1000.0 / mw_np
                    elif self.config.uptake_conversion_mode == "per_volume":
                        vol_np = batch.cell_volume[mask].detach().cpu().numpy()
                        pred_np = pred_np * 1000.0 * vol_np / mw_np
                        gt_np   = gt_np   * 1000.0 * vol_np / mw_np
    ...
    def on_train_epoch_start(self):
        ...
        # Lightning's _set_sampler_epoch only calls set_epoch() on the BASE sampler
        # (dataloader.batch_sampler.sampler), not on the batch sampler itself.
        # We must call it ourselves so AtomBucketBatchSampler rebuilds batches
        # for the new epoch BEFORE Lightning calls len() and iter() on it.
        # Without this, __len__ and __iter__ would see stale epoch=0 and never
        # produce different shuffles.
        ...
        # Lightning computes val_check_batch once from epoch 0's batch count.
        # With AtomBucketBatchSampler (bucketed packing + drop_last), different
        # shuffles produce different batch counts per epoch. The stale
        # val_check_batch makes the modular check silently skip validation.
        # Setting val_check_batch=inf forces Lightning to use is_last_batch instead.
        if getattr(self.trainer, "val_check_batch", None) is not None:
    ...
    def on_train_epoch_end(self):
        ...
        print(f"[DEBUG][rank={rank}] training epoch {epoch} FINISHED  "
              f"(val_check_batch={getattr(self.trainer, 'val_check_batch', '?')})")
    ...
    def on_validation_epoch_end(self):
        ...
        epoch = self.trainer.current_epoch
        rank = getattr(self.trainer, "global_rank", 0)
        print(f"[DEBUG][rank={rank}] validation epoch {epoch} FINISHED")
    ...
    def data_transform(self, data: BaseData):
        ...
        # Ensure partial_charges exists and has correct length.
        # Some CIFs may lack charges, or _read_partial_charges may return a
        # different count than ase_read atoms (e.g. symmetry expansion mismatch).
        if not hasattr(data, "partial_charges") or len(data.partial_charges) != data.natoms:
            if hasattr(data, "partial_charges") and len(data.partial_charges) != data.natoms:
                import logging
                logging.getLogger(__name__).warning(
                    f"partial_charges length {len(data.partial_charges)} != natoms {data.natoms} "
                    f"for sample {getattr(data, 'id', '?')}; resetting to zeros"
                )
            data.partial_charges = torch.zeros(data.natoms, dtype=torch.float32, device=data.pos.device)

        if self.config.node_sh_targets and hasattr(self, "_sh_norm_stats"):
            for sh_target in self.config.node_sh_targets:
                if hasattr(data, sh_target) and sh_target in self._sh_norm_stats:
                    mean, std = self._sh_norm_stats[sh_target]
                    val = getattr(data, sh_target)
                    if val.dtype != mean.dtype:
                        mean = mean.to(val.dtype)
                        std = std.to(val.dtype)
                    setattr(data, sh_target, (val - mean) / std)
    ...
    def _task_dataset(
        ...
        # Convert uptake units (before normalization)
        if self.config.uptake_conversion_mode == "per_cell":
            dataset = DT.transform(dataset, _convert_uptake_mmolg_to_per_cell)
        elif self.config.uptake_conversion_mode == "per_volume":
            dataset = DT.transform(dataset, _convert_uptake_mmolg_to_per_volume)
        ...
        _transform_map: dict[str, Callable[..., BaseData]] = {
            ...
            "adsorption_mof_db_anion": _identity_transform,
            "adsorption_mof_db_anion_co2sh": _identity_transform,
        }
        ...
        if training:
            ...
            if self.config.node_sh_targets and not hasattr(self, "_sh_norm_stats"):
                self._compute_sh_normalization(dataset, config)
    ...
    def _compute_sh_normalization(self, dataset, config):
        import json
        stats_file = None
        if hasattr(config, "src") and config.src is not None:
            candidate = config.src.parent.parent / "normalization_stats.json"
            if candidate.exists():
        ...
        if stats_file is not None:
            ...
            train_stats = norm_data.get("train", {})
            if train_stats and "mean" in train_stats and "std" in train_stats:
                self._sh_norm_stats = {}
                for sh_target in self.config.node_sh_targets:
                    mean = torch.tensor(train_stats["mean"], dtype=torch.float32)
                    std = torch.tensor(train_stats["std"], dtype=torch.float32).clamp(min=1e-8)
                    self._sh_norm_stats[sh_target] = (mean, std)
                log.info("Loaded SH normalization from %s", stats_file)
                return

        log.info("Computing SH normalization OTF over %d samples...", len(dataset))
        n_sh = (self.config.node_sh_lmax + 1) ** 2
        running_sum = np.zeros(n_sh, dtype=np.float64)
        running_sq = np.zeros(n_sh, dtype=np.float64)
        total_atoms = 0

        for i in range(len(dataset)):
            sample = dataset[i]
            for sh_target in self.config.node_sh_targets:
                if hasattr(sample, sh_target) and hasattr(sample, "has_co2_sh"):
                    has_sh = sample.has_co2_sh
                    if torch.is_tensor(has_sh):
                        has_sh = has_sh.item()
                    if has_sh:
                        coeffs = getattr(sample, sh_target)
                        if torch.is_tensor(coeffs):
                            coeffs = coeffs.numpy()
                        running_sum += coeffs.sum(axis=0)
                        running_sq += (coeffs ** 2).sum(axis=0)
                        total_atoms += coeffs.shape[0]

        self._sh_norm_stats = {}
        if total_atoms > 0:
            mean = running_sum / total_atoms
            var = running_sq / total_atoms - mean ** 2
            std = np.sqrt(np.maximum(var, 1e-12))
            for sh_target in self.config.node_sh_targets:
                self._sh_norm_stats[sh_target] = (
                    torch.tensor(mean, dtype=torch.float32),
                    torch.tensor(std, dtype=torch.float32).clamp(min=1e-8),
                )
            log.info("SH normalization computed: %d atoms from dataset", total_atoms)
        else:
            log.warning("No samples with SH data found; skipping SH normalization")
    ...
    def construct_output_heads(self):
        ...
        self.node_sh_outputs = TypedModuleDict(
            {
                target: TypedModuleList(
                    [
                        EqV2NodeSHHead(
                            self.backbone,
                            lmax_target=self.config.node_sh_lmax,
                            output_name=target,
                        )
                        for _ in range(1)
                    ]
                )
                for target in self.config.node_sh_targets
            },
            key_prefix="ft_sh_",
        )

        # Binary classification heads for high-value detection
        if self.config.classification_targets:
    ...
    def forward(self, data: BaseData):
        ...
        for target, modules in self.node_sh_outputs.items():
            results = [module(output_head_input) for module in modules]
            preds[target] = results[0][target]
    ...
    def _gaussian_nll_loss(
        ...
        y_pred_log_var = y_pred_log_var.clamp(min=-10, max=10)
    ...
    def _graph_level_loss(
        ...
        if roi_penalty != 1.0:
            try:
                ...
                # ROI bounds are in mmol/g; convert back if training in converted units
                if self.config.uptake_conversion_mode != "none" and target in UPTAKE_TARGETS:
                    mw = batch.cell_mol_weight.unsqueeze(-1) if gt_denorm.dim() > 1 else batch.cell_mol_weight
                    if self.config.uptake_conversion_mode == "per_cell":
                        gt_denorm = gt_denorm * 1000.0 / mw
                    elif self.config.uptake_conversion_mode == "per_volume":
                        vol = batch.cell_volume.unsqueeze(-1) if gt_denorm.dim() > 1 else batch.cell_volume
                        gt_denorm = gt_denorm * 1000.0 * vol / mw
    ...
    def _node_sh_loss(
        ...
        y_pred = preds[target]
        y_true = getattr(batch, target).to(y_pred.dtype)

        has_sh = getattr(batch, "has_co2_sh", None)
        if has_sh is None:
            return torch.tensor(0.0, device=self.device)

        if has_sh.dim() == 0:
            has_sh = has_sh.unsqueeze(0)
        atom_mask = has_sh[batch.batch]

        if atom_mask.sum() == 0:
            return torch.tensor(0.0, device=self.device)

        if self.config.node_sh_loss_type == "mse":
            loss = F.mse_loss(y_pred, y_true, reduction="none")
        else:
            loss = F.l1_loss(y_pred, y_true, reduction="none")

        if self.config.node_sh_l_weights is not None:
            n_sh = (self.config.node_sh_lmax + 1) ** 2
            weight_vec = torch.zeros(n_sh, device=loss.device, dtype=loss.dtype)
            idx = 0
            for l_deg in range(self.config.node_sh_lmax + 1):
                w = self.config.node_sh_l_weights[l_deg] if l_deg < len(self.config.node_sh_l_weights) else 1.0
                for _ in range(2 * l_deg + 1):
                    weight_vec[idx] = w
                    idx += 1
            loss = loss * weight_vec.unsqueeze(0)

        loss = loss * atom_mask.unsqueeze(-1).float()
        n_valid = atom_mask.sum().clamp(min=1)
        return loss.sum() / (n_valid * y_pred.shape[-1])
    ...
    def compute_losses(
        ...
        for sh_target in self.config.node_sh_targets:
            if sh_target in preds:
                sh_loss = self._node_sh_loss(batch, preds, sh_target)
                sh_loss = self.config.node_sh_loss_coefficient * sh_loss
                if self.training:
                    self.log(f"{sh_target}_loss", sh_loss)
                per_target_losses.append(sh_loss)
    ...
    def train_metrics_provider(
        ...
        # Convert uptake back to mmol/g for metric reporting
        if self.config.uptake_conversion_mode != "none" and prop in UPTAKE_TARGETS:
            pred, target = _inverse_convert_uptake(pred, target, batch, task_mask,
                                                   self.config.uptake_conversion_mode)
    ...
    def _asymmetric_focal_bce_loss(
        ...
        """
        # Numerically stable BCE using log-sum-exp trick (handles extreme logits)
        bce = F.binary_cross_entropy_with_logits(logits, targets, reduction="none")

        # Clamp probs away from 0/1 for stable focal weights
        probs = torch.sigmoid(logits).clamp(min=1e-7, max=1.0 - 1e-7)

        # Focal + asymmetric weighting per class
        focal_weight = torch.where(
            targets == 1,
            alpha_pos * (1 - probs).pow(gamma),   # Hard positives
            alpha_neg * probs.pow(gamma),           # Hard negatives
        )
