...
from hmof_preprocess import DataLoader, _configure_logging

LOGGER = logging.getLogger("hmof_lmdb")
...
def _derive_all_indices_from_csv(dataset_path: str) -> List[int]:
    ...
    import pandas as pd
    csv_path = Path(dataset_path) / "raw" / "all_MOFs_screening_data_cleaned.csv"
    df = pd.read_csv(csv_path, usecols=[0])
    return list(range(len(df)))


def _make_splits(indices, ratio=(8, 1, 1), seed=42) -> Dict[str, List[int]]:
