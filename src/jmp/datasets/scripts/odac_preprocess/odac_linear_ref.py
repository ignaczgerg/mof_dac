...
def _compute_mean_std(args: argparse.Namespace):
    ...
    energies = [y for y, na, _ in outputs]
    num_atoms = [na for y, na, _ in outputs]
    forces = np.concatenate([f.numpy().reshape(-1, 3) for _, _, f in outputs], axis=0)
    ...
    print(f"force_mean: {force_mean}, force_std: {force_std}")
