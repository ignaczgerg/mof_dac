...
class LmdbDataset(Dataset[T], ContextDecorator):
    ...
    def __getitem__(self, idx: int):
        ...
        if fusion_cfg_enabled:
            feature_name = getattr(self.args, "fusion_feature_name", None)

            if feature_name is not None:
                if hasattr(data_object, feature_name):
                    value = getattr(data_object, feature_name)
                    data_object.mof_descriptor = torch.tensor([[value]], dtype=torch.float32)
                else:
                    raise KeyError(
                        f"LMDB entry does not contain descriptor '{feature_name}'. "
                        f"Available keys: {data_object.__dict__.keys()}"
                    )
