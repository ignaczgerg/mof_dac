...
class LmdbDataset(Dataset[T], ContextDecorator):
    ...
    # Added to support DatasetProtocol
    @property
    ...
    def __init__(
        ...
        # NOTE: Sampling is now handled by DatasetSampleNConfig in wrap_common_dataset()
        # This monstrosity is disabled to avoid double-sampling.
        # The shuffled_indices are set to the full range... 
        # actual sampling happens in a galaxy far far away.
    ...
    def __getitem__(self, idx: int):
        ...
        db_idx = bisect.bisect(self.keylen_cumulative, lmdb_idx)
        # Extract index of element within that db.
        el_idx = lmdb_idx
        if db_idx != 0:
            el_idx = lmdb_idx - self.keylen_cumulative[db_idx - 1]
        ...
        if data_object_pickled is None:
            raise KeyError(
                f"Key {key=} not found in {env=}. {el_idx=} {db_idx=} {lmdb_idx=}"
            )
        ...
        # -------------------------------
        # Add MOF-level descriptor (PLD, LCD, etc.)
        # -------------------------------
        fusion_cfg_enabled = getattr(self.args, "enable_feature_fusion", False)

        if fusion_cfg_enabled:
            feature_names = getattr(self.args, "fusion_feature_names", None)

            if feature_names is not None:
                values = []
                for fname in feature_names:
                    if hasattr(data_object, fname):
                        values.append(float(getattr(data_object, fname)))
                    else:
                        raise KeyError(
                            f"LMDB entry does not contain descriptor '{fname}'. "
                            f"Available keys: {list(data_object.keys()) if hasattr(data_object, 'keys') else dir(data_object)}"
                        )
                data_object.mof_descriptor = torch.tensor([values], dtype=torch.float32)  # [1, F]
