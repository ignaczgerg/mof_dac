...
class PretrainLmdbDataset(Dataset[BaseData]):
    ...
    def __init__(
        ...
        # # Note: uncomment the following lines to revert to EqV2 max_instances behavior 
        # if self.config.is_train and self.config.max_samples is not None:
        #     self.num_samples = self.config.max_samples
        # ############

        if not self.config.is_train and self.config.max_samples is not None:
            self.num_samples = self.config.max_samples
    ...
    def __getitem__(self, idx):
        ...
        if "pos" in data_object:
            data_object.center = data_object.pos.mean(dim=0, keepdim=True)
...
class PermutedDataset(PretrainLmdbDataset):
    def __init__(self, base_dataset, num_perms=4):
    ...
    @property
    def atoms_metadata(self) -> np.ndarray:
        return self.base_dataset.atoms_metadata

    def __len__(self):
        return len(self.base_dataset) * self.num_perms
    
    def set_epoch(self, epoch: int):
        print(f"Setting epoch for PermutedDataset: {epoch}")
        self.epoch = epoch

    def __getitem__(self, idx):
        base_idx = idx // self.num_perms
        perm_idx = idx % self.num_perms

        data = self.base_dataset[base_idx].clone()
        n_atoms = data.pos.shape[0]

        # Generate permutation
        seed_idx = base_idx * 1000 + perm_idx + self.epoch * 100000 if self.base_dataset.config.is_train else base_idx * 1000 + perm_idx
        perm = torch.randperm(n_atoms, generator=torch.Generator().manual_seed(seed_idx))

        # Apply to all atom-level features
        data.pos = data.pos[perm]
        if hasattr(data, "atomic_numbers"):
            data.atomic_numbers = data.atomic_numbers[perm]
        if hasattr(data, "force"):
            data.force = data.force[perm]
        if hasattr(data, "tags"):
            data.tags = data.tags[perm]
        if hasattr(data, "fixed"):
            data.fixed = data.fixed[perm]
        # delete graph-related fields if OTF graph
        if hasattr(data, "edge_index"): del data.edge_index
        if hasattr(data, "cell_offsets"): del data.cell_offsets
        return data
