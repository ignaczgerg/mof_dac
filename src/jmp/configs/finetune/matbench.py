...
MATBENCH_DATASETS: list[tuple[DC.MatbenchDataset, int, bool]] = [
    ("mp_is_metal", 3, False),
    ("jdft2d", 3, False),
    ("phonons", 8, True),
    ("dielectric", 8, True),
...
def jmp_l_matbench_config_(
    ...
    batch_size = None
    amp = None
    for target_name, batch_size, amp in MATBENCH_DATASETS:
    ...
    # Set up dataset
    config.train_dataset = DC.matbench_config(dataset, base_path, "train", fold, args=args)
    config.val_dataset = DC.matbench_config(dataset, base_path, "val", fold, args=args)
    config.test_dataset = DC.matbench_config(dataset, base_path, "test", fold, args=args)
