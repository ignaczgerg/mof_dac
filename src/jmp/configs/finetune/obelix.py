...
from jmp.tasks.finetune.obelix import ObelixConfig, ObelixTarget
...
def equiformer_v2_obelix_config_(
    config: ObelixConfig,
    base_path: Path,
    ...
    config.train_dataset = DC.obelix_config(base_path, "train", args=args)
    config.val_dataset   = DC.obelix_config(base_path, "val",   args=args)
    config.test_dataset  = DC.obelix_config(base_path, "test",  args=args)
