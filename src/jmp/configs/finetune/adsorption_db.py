...
# these are just placeholder values. The reals stats are computed on the fly. 
# this is legacy and needs to be removed in the future. TODO
STATS_MOF_ANION: dict[str, NC] = {
"qst_co2": NC(mean=0.0, std=1.0, normalization_type="standard"),
"qst_h2o": NC(mean=0.0, std=1.0, normalization_type="standard"),
"kh_co2": NC(mean=0.0, std=1.0, normalization_type="standard"),
"kh_h2o": NC(mean=0.0, std=1.0, normalization_type="standard"),
"selectivity_co2_h2o": NC(mean=0.0, std=1.0, normalization_type="standard"),
"co2_uptake": NC(mean=0.0, std=1.0, normalization_type="standard"),
"qst_n2": NC(mean=0.0, std=1.0, normalization_type="standard"),
"kh_n2": NC(mean=0.0, std=1.0, normalization_type="standard"),
"selectivity_co2_n2": NC(mean=0.0, std=1.0, normalization_type="standard"),
"n2_uptake": NC(mean=0.0, std=1.0, normalization_type="standard"),
}

LOG_STATS_MOF_ANION: dict[str, NC] = {
"qst_co2": NC(mean=0.0, std=1.0, normalization_type="log"),
"qst_h2o": NC(mean=0.0, std=1.0, normalization_type="log"),
"kh_co2": NC(mean=0.0, std=1.0, normalization_type="log"),
"kh_h2o": NC(mean=0.0, std=1.0, normalization_type="log"),
"selectivity_co2_h2o": NC(mean=0.0, std=1.0, normalization_type="log"),
"co2_uptake": NC(mean=0.0, std=1.0, normalization_type="log"),
"qst_n2": NC(mean=0.0, std=1.0, normalization_type="log"),
"kh_n2": NC(mean=0.0, std=1.0, normalization_type="log"),
"selectivity_co2_n2": NC(mean=0.0, std=1.0, normalization_type="log"),
"n2_uptake": NC(mean=0.0, std=1.0, normalization_type="log"),
}

STATS_ARAMCO: dict[str, NC] = {
"qst_co2": NC(mean=0.0, std=1.0, normalization_type="standard"),
"qst_h2o": NC(mean=0.0, std=1.0, normalization_type="standard"),
"kh_co2": NC(mean=0.0, std=1.0, normalization_type="standard"),
"kh_h2o": NC(mean=0.0, std=1.0, normalization_type="standard"),
...
MUTUALLY_EXCLUSIVE_DATASETS = [
    {"adsorption_db1_merged", "adsorption_mof_db_1_charges"},
    {"adsorption_db2_merged", "adsorption_mof_db_2_charges"},
    {"adsorption_mof_db_anion", "adsorption_mof_db_anion_charges"},
]

def _validate_no_conflicting_datasets(dataset_names: list[str]):
    name_set = set(dataset_names)
    for exclusive_pair in MUTUALLY_EXCLUSIVE_DATASETS:
        overlap = name_set & exclusive_pair
        if len(overlap) > 1:
            raise ValueError(
                f"Cannot use datasets {overlap} together — they are the same data "
                f"with/without charges. Pick one."
            )
...
def _build_normalization_map(
    ...
    stats_source = {
        ...
        "adsorption_mof_db_anion": {"standard": STATS_MOF_ANION, "log": LOG_STATS_MOF_ANION},
        "adsorption_mof_db_anion_co2sh": {"standard": STATS_MOF_ANION, "log": LOG_STATS_MOF_ANION},
        "adsorption_mof_db_1_charges": {"standard": STATS, "log": LOG_STATS},
        "adsorption_mof_db_2_charges": {"standard": STATS_DB2, "log": LOG_STATS_DB2},
        "adsorption_mof_db_anion_charges": {"standard": STATS_MOF_ANION, "log": LOG_STATS_MOF_ANION},
        "adsorption_aramco": {"standard": STATS_ARAMCO, "log": LOG_STATS_ARAMCO},
    }
    ...
    from jmp.tasks.finetune.adsorption_db import GEOMETRIC_PROPERTY_NAMES
    ...
    for i, t in enumerate(targets):
        ...
        if t not in src_dict:
            if t in GEOMETRIC_PROPERTY_NAMES:
                # Geometric targets: use placeholder; OTF normalization will overwrite
                nc = NC(mean=0.0, std=1.0, normalization_type=ntype)
            else:
                raise KeyError(
...
def jmp_l_adsorption_db_config_(
    ...
    for idx, target in enumerate(targets):
        ...
        stats_map = LOG_STATS if args.normalization_type[idx] in ["log", "log1p"] else STATS
...
def jmp_l_adsorption_db_multi_task_config_(
    ...
    _validate_no_conflicting_datasets(dataset_names)
    ...
    all_tasks = {
        ...
        "adsorption_mof_db_anion": TaskConfig(
            name="adsorption_mof_db_anion",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_anion/lmdb/train",
                sample_n=DatasetSampleNConfig(
                    sample_n=train_samples_limit,
                    seed=args.seed
                ) if train_samples_limit is not None else None,
                args=args,
            ) if "adsorption_mof_db_anion" in dataset_names else None,
            val_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_anion/lmdb/val",
                sample_n=DatasetSampleNConfig(
                    sample_n=val_samples_limit,
                    seed=args.seed
                ) if val_samples_limit is not None else None,
            ...
            test_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_anion/lmdb/test",
                sample_n=DatasetSampleNConfig(
                    sample_n=test_samples_limit,
                    seed=args.seed
                ) if test_samples_limit is not None else None,
            ...
            graph_scalar_targets=config.graph_scalar_targets,
            graph_scalar_loss_coefficients=config.graph_scalar_loss_coefficients,
            normalization=_build_normalization_map("adsorption_mof_db_anion", targets, args),
        ),

        "adsorption_mof_db_1_charges": TaskConfig(
            name="adsorption_mof_db_1_charges",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_1_charges/lmdb/train",
                sample_n=DatasetSampleNConfig(
                    sample_n=train_samples_limit,
                    seed=args.seed
                ) if train_samples_limit is not None else None,
                args=args,
            ) if "adsorption_mof_db_1_charges" in dataset_names else None,
            val_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_1_charges/lmdb/val",
                sample_n=DatasetSampleNConfig(
                    sample_n=val_samples_limit,
                    seed=args.seed
                ) if val_samples_limit is not None else None,
            ...
            test_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_1_charges/lmdb/test",
                sample_n=DatasetSampleNConfig(
                    sample_n=test_samples_limit,
                    seed=args.seed
                ) if test_samples_limit is not None else None,
            ...
            graph_scalar_targets=config.graph_scalar_targets,
            graph_scalar_loss_coefficients=config.graph_scalar_loss_coefficients,
            normalization=_build_normalization_map("adsorption_mof_db_1_charges", targets, args),
        ),

        "adsorption_mof_db_2_charges": TaskConfig(
            name="adsorption_mof_db_2_charges",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_2_charges/lmdb/train",
                sample_n=DatasetSampleNConfig(
                    sample_n=train_samples_limit,
                    seed=args.seed
                ) if train_samples_limit is not None else None,
                args=args,
            ) if "adsorption_mof_db_2_charges" in dataset_names else None,
            val_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_2_charges/lmdb/val",
                sample_n=DatasetSampleNConfig(
                    sample_n=val_samples_limit,
                    seed=args.seed
                ) if val_samples_limit is not None else None,
            ...
            test_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_2_charges/lmdb/test",
                sample_n=DatasetSampleNConfig(
                    sample_n=test_samples_limit,
                    seed=args.seed
                ) if test_samples_limit is not None else None,
            ...
            graph_scalar_targets=config.graph_scalar_targets,
            graph_scalar_loss_coefficients=config.graph_scalar_loss_coefficients,
            normalization=_build_normalization_map("adsorption_mof_db_2_charges", targets, args),
        ),

        "adsorption_mof_db_anion_charges": TaskConfig(
            name="adsorption_mof_db_anion_charges",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_anion_charges/lmdb/train",
                sample_n=DatasetSampleNConfig(
                    sample_n=train_samples_limit,
                    seed=args.seed
                ) if train_samples_limit is not None else None,
                args=args,
            ) if "adsorption_mof_db_anion_charges" in dataset_names else None,
            val_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_anion_charges/lmdb/val",
                sample_n=DatasetSampleNConfig(
                    sample_n=val_samples_limit,
                    seed=args.seed
                ) if val_samples_limit is not None else None,
            ...
            test_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_anion_charges/lmdb/test",
                sample_n=DatasetSampleNConfig(
                    sample_n=test_samples_limit,
                    seed=args.seed
                ) if test_samples_limit is not None else None,
            ...
            graph_scalar_targets=config.graph_scalar_targets,
            graph_scalar_loss_coefficients=config.graph_scalar_loss_coefficients,
            normalization=_build_normalization_map("adsorption_mof_db_anion_charges", targets, args),
        ),

        "adsorption_aramco": TaskConfig(
            name="adsorption_aramco",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= Path("/ibex/project/c2261/datasets/aramco_mofs/output_data/train"),
                sample_n=DatasetSampleNConfig(
                    sample_n=train_samples_limit,
                    seed=args.seed
                ) if train_samples_limit is not None else None,
                args=args,
            ) if "adsorption_aramco" in dataset_names else None,
            val_dataset=FinetuneLmdbDatasetConfig(
                src= Path("/ibex/project/c2261/datasets/aramco_mofs/output_data/val"),
                sample_n=DatasetSampleNConfig(
                    sample_n=val_samples_limit,
                    seed=args.seed
                ) if val_samples_limit is not None else None,
            ...
            test_dataset=FinetuneLmdbDatasetConfig(
                src= Path("/ibex/project/c2261/datasets/aramco_mofs/output_data/test"),
                sample_n=DatasetSampleNConfig(
                    sample_n=test_samples_limit,
                    seed=args.seed
                ) if test_samples_limit is not None else None,
            ...
            graph_scalar_targets=config.graph_scalar_targets,
            graph_scalar_loss_coefficients=config.graph_scalar_loss_coefficients,
            normalization=_build_normalization_map("adsorption_aramco", targets, args),
        ),
...
def equiformer_v2_adsorption_db_multitask_config_(
    ...
    config.mof_feature_fusion = MOFFeatureConfig(
        ...
        feature_names=args.fusion_feature_names,
    ...
    anion_path = c2261_path / "all-adsorption/mof_db_anion"
    if anion_path.exists():
        all_tasks["adsorption_mof_db_anion"] = TaskConfig(
            name="adsorption_mof_db_anion",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_anion/lmdb/train",
                sample_n=DatasetSampleNConfig(
    ...
    db1_charges_path = c2261_path / "all-adsorption/mof_db_1_charges"
    if db1_charges_path.exists():
        all_tasks["adsorption_mof_db_1_charges"] = TaskConfig(
            name="adsorption_mof_db_1_charges",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_1_charges/lmdb/train",
                sample_n=DatasetSampleNConfig(
    ...
    db2_charges_path = c2261_path / "all-adsorption/mof_db_2_charges"
    if db2_charges_path.exists():
        all_tasks["adsorption_mof_db_2_charges"] = TaskConfig(
            name="adsorption_mof_db_2_charges",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_2_charges/lmdb/train",
                sample_n=DatasetSampleNConfig(
    ...
    anion_charges_path = c2261_path / "all-adsorption/mof_db_anion_charges"
    if anion_charges_path.exists():
        all_tasks["adsorption_mof_db_anion_charges"] = TaskConfig(
            name="adsorption_mof_db_anion_charges",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= c2261_path / "all-adsorption/mof_db_anion_charges/lmdb/train",
                sample_n=DatasetSampleNConfig(
    ...
    aramco_path = Path("/ibex/project/c2261/datasets/aramco_mofs/output_data")
    if aramco_path.exists():
        all_tasks["adsorption_aramco"] = TaskConfig(
            name="adsorption_aramco",
            train_dataset=FinetuneLmdbDatasetConfig(
                src= aramco_path / "train",
                sample_n=DatasetSampleNConfig(
                    sample_n=train_samples_limit,
                    seed=args.seed
                ) if train_samples_limit is not None else None,
