...
def get_configs(dataset_name: str, targets: list[str], args):
    ...
    elif getattr(args, "pretrained_ckpt_path", None):
        print(f"Overriding pretrained checkpoint path =================")
        ckpt_path = Path(args.pretrained_ckpt_path)
    ...
    _dataset_base_path_override = getattr(args, "dataset_base_path", None)
    if _dataset_base_path_override:
        base_path = Path(_dataset_base_path_override)
    else:
    ...
    #     init_model = AdsorptionDbModel
    ...
    elif dataset_name in [ "adsorption_db1_merged", "adsorption_db2_merged", "adsorption_cof_db2", "adsorption_mof_db_anion", "adsorption_mof_db_anion_co2sh", "adsorption_mof_db_1_charges", "adsorption_mof_db_2_charges", "adsorption_mof_db_anion_charges", "adsorption_aramco"]:
        if dataset_name == "adsorption_aramco":
            base_path = Path("/ibex/project/c2261/datasets/aramco_mofs/output_data")
        if _dataset_base_path_override:
            print(f"Overriding dataset base_path =================")
            base_path = Path(_dataset_base_path_override)
        config = AdsorptionDbConfig.draft()
        ...
        init_model = AdsorptionDbModel
        config.uptake_conversion_mode = getattr(args, "uptake_conversion_mode", "none")

        if getattr(args, "node_sh_targets", None):
            config.node_sh_targets = args.node_sh_targets
            config.node_sh_lmax = getattr(args, "node_sh_lmax", 4)
            config.node_sh_loss_coefficient = getattr(args, "node_sh_loss_coefficient", 1.0)
...
def define_missing_keys_early_fusion(model, backbone_state):
    ...
    new_layer_prefixes = ["mof_embedding_proj", "mof_rbf_encoder", "charge_rbf_encoder", "charge_embedding_proj"]
...
def load_checkpoint(model, config, args):
    ...
    if args.model_name == "gemnet":
        ...
        if (args.enable_feature_fusion and args.fusion_type == "early") or getattr(args, "use_charge_embedding", False):
