...
parser.add_argument("--dataset_base_path", type=str, default=None,
                    help="Override the dataset base path. When set, overrides the per-dataset "
                         "default base_path (e.g. <root_path>/datasets/<name>/ or any hardcoded path).")
parser.add_argument("--pretrained_ckpt_path", type=str, default=None,
                    help="Override the pretrained backbone checkpoint path. When set, this absolute "
                         "path is used directly instead of resolving CHECKPOINT_TAG_MAPPING[--checkpoint_tag] "
                         "under <root_path>/checkpoints/EquiformerV2/.")
...
parser.add_argument("--run_tag", type=str, default=None,
    help="Stable run identifier used as config.id (and wandb run id). "
         "When set, the checkpoint dir at <logging_path>/lightning_logs/<run_tag>/checkpoints "
         "is reused across resubmissions: training resumes from last.ckpt or step_latest.ckpt "
         "(whichever is newer). Required for the short-job/watchdog workflow.")
parser.add_argument("--save_every_n_train_steps", type=int, default=None,
    help="Save a step-based checkpoint every N training steps. Required for short-job "
         "resume when an epoch is longer than the SLURM walltime.")
parser.add_argument("--done_sentinel_dir", type=str, default=None,
    help="Directory to write <run_tag>.done after trainer.test(). "
         "Defaults to <logging_path>/lightning_logs/<run_tag>/. The watchdog polls this.")
...
def _resolve_resume_ckpt(args) -> str | None:
    if not args.run_tag:
        return None
    ckpt_dir = (
        os.path.join(args.logging_path, "lightning_logs", args.run_tag, "checkpoints")
    )
    candidates = [
        os.path.join(ckpt_dir, "last.ckpt"),
        os.path.join(ckpt_dir, "step_latest.ckpt"),
    ]
...
def run(config: FinetuneConfigBase, model_cls: type[FinetuneModelBase]) -> None:
    ...
    resume_path = _resolve_resume_ckpt(args)
    if resume_path is not None and is_rank_zero():
        print(f"[finetune] resuming from checkpoint: {resume_path}")

    if not args.scratch and resume_path is None:
    ...
    if args.run_tag and args.save_every_n_train_steps:
        ckpt_dir = os.path.join(
            args.logging_path, "lightning_logs", args.run_tag, "checkpoints"
        )
        os.makedirs(ckpt_dir, exist_ok=True)
        _step_every = args.save_every_n_train_steps
        model.register_callback(lambda: ModelCheckpoint(
            dirpath=ckpt_dir,
            filename="step_latest",
            every_n_train_steps=_step_every,
            save_top_k=1,
            monitor=None,
            save_last=False,
    ...
    if args.run_tag and is_rank_zero():
        sentinel_dir = args.done_sentinel_dir or os.path.join(
            args.logging_path, "lightning_logs", args.run_tag
        )
        os.makedirs(sentinel_dir, exist_ok=True)
        sentinel = os.path.join(sentinel_dir, f"{args.run_tag}.done")
        with open(sentinel, "w") as f:
            f.write("ok\n")
        print(f"[finetune] wrote sentinel: {sentinel}")
...
runner = Runner(run)
runner(configs, reset_id=not bool(args.run_tag))
