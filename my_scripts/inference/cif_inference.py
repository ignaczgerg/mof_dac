...
def main():
    ...
    # --- Geometric property pre-computation (for feature fusion models) ---
    geo_props = {}
    fusion_feature_names = None
    mof_fusion_cfg = getattr(cfg, "mof_feature_fusion", None) or {}
    if isinstance(mof_fusion_cfg, dict):
        needs_geo = mof_fusion_cfg.get("enabled", False)
        fusion_feature_names = mof_fusion_cfg.get("feature_names") if needs_geo else None
    else:
        needs_geo = getattr(mof_fusion_cfg, "enabled", False)
        fusion_feature_names = getattr(mof_fusion_cfg, "feature_names", None) if needs_geo else None
    # Also detect from backbone config (early fusion stores it there)
    if not needs_geo:
        bb_fusion = getattr(cfg.backbone, "mof_feature_fusion", None) or {}
        if isinstance(bb_fusion, dict):
            needs_geo = bb_fusion.get("enabled", False)
            fusion_feature_names = bb_fusion.get("feature_names") if needs_geo else None
        else:
            needs_geo = getattr(bb_fusion, "enabled", False)
            fusion_feature_names = getattr(bb_fusion, "feature_names", None) if needs_geo else None
    if needs_geo:
        if fusion_feature_names is None:
            fusion_feature_names = ["pld", "lcd", "gcd", "unitcell_volume", "density", "asa", "av", "nav"]
        print(f"[INFO] mof_feature_fusion enabled: computing geometric properties ({', '.join(fusion_feature_names)})")
        ...
        print(f"[INFO] Geometric properties computed for {len(geo_props)}/{len(filtered_paths)} CIFs.")
